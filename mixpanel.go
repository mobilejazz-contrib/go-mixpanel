...
// CreateProfile creates a "People" profile in Mixpanel with a distinctID
...
func (m *Mixpanel) CreateProfile(distinctID string, ip string, properties map[string]interface{}) error {
	return m.engage(distinctID, ip, "$set", properties)
}

// SetPropertiesOnProfileOnce sets properties that are not already set in the profile
// that is referenced by the distinctID
...
func (m *Mixpanel) engage(distinctID string, ip string, op string, properties interface{}) error {
	var data map[string]interface{} = make(map[string]interface{})
